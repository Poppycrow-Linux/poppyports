...
def human_fsize(
    path,
):  ## TODO: make a separate py file for nicities like colors or this function
...
def log(clr, *args):
    if (
        supressnonerrorlogs and (clr in {Colors.SUCCESS, Colors.ERROR, Colors.WARNING})
    ) or not (supressnonerrorlogs):
...
class BuildContext:  # https://wiki.alpinelinux.org/wiki/APKBUILD_Reference
    ...
    def __init__(self, builddir, portdir, recipe):
        ...
        self.LIBC = "glibc"  # switch to musl to break everything TODO: package musl to introduce more bugs to fix later
    ...
    def sh(self, *args, cwd=None, shell=False):
        ...
        log(
            Colors.SH_COMMAND, f"+$ {' '.join(args) if isinstance(cmd, tuple) else cmd}"
        )
    ...
    with open(path, "r") as f:
        ...
        if missing_keys:
            raise InvalidRecipeError(
                f"This recipe is missing the {', '.join(missing_keys)} key(s)!"
            )
        return recipe_def
...
if __name__ == "__main__":
    ...
    if (
        os.path.getsize(CONFIGFILEPATH) == 0
    ):  ## create and write default config if it is missing
        configparser.write(
            configfile
    ...
    if os.path.exists(outpath) and not rebuild:
        log(
            Colors.WARNING,
            f"Skipping build as {outpath} already exists. If you need to rebuild, pass the -rebuild flag to force rebuilding.",
        )
        sys.exit(0)
    ...
    if "sha256sum" in recipe:
        ...
        log(
            Colors.WARNING,
            f"//// SHA256 checksum not found in recipe {recipe['pkgname']}, extracting without checks. ////",
        )
    ...
    apkcmd.extend(
        [
            "-I",
            f"name:{recipe['pkgname']}",
            "-I",
            f"version:{recipe['pkgver']}",
            "-I",
